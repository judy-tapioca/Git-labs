...
class Example {
    MyString s1;  // member variable object allocated inside example
...
void test() {
    ...
    // TODO: will in ex.s1 be garbage?
           // no, because Example has memeber variable that is an object of another clss
           //  so the contructor of Example automatically calls the contructor of mystring
           // mystring has a default contructor that initializes to nullptr
           

    ex.set_str("test string");
    Example ex2 = ex; // deep copy has a copy contructor
    // ex2 has itss own memory allocation
    // TODO: will all this operations work correctly? 
    
}  // TODO: will there be memory leaks?
   // no memory leak because they point at differnt memory address 
   //each has its own destructor 
 
void test_pass_by_value(MyString s1) { 
 s1.set_new_string("test2");// local modified only
} // out of scope 
...
void foo() {
 ...
 // s1 is created,s1.data points to "123" on the heap
 //deeep copy, own memory
 // printss 123 becuae the set new string freed its own locak copy 
 //so si in foo is not affected 
}
 void test_wrapper() {
    std::cout << "\n=== TextWrapper tests ===\n";

    TextWrapper tw1(MyString("visa todaaaaay visa today visa today visa today visa today visa today visa today visa today"), 10);
    tw1.print_wrapped();

    TextWrapper tw2(MyString("food  lovers"), 10);
    tw2.print_wrapped();

    TextWrapper tw3(MyString("hippopotamus"), 5); // does not cut word in the middle
    tw3.print_wrapped();

    TextWrapper tw4(MyString("hi how are you"), 10);
    tw4.print_wrapped();
}
...
int main()
{
    ...
    {
        Rect rect; // Constructs (creates) an object of type Rect using the default constructor (It is a constructor call, but it calls an implicit (compiler-generated))
    } 
    ...
    { 
       ...
       // C++ gave  Rect() for free, but the moment we wrote Rect(int,int,int,int)
       // it deleted it. So Rect rect; would crash. We add it back manually here.
    }
...
std::cout << "\n== TASK 1.4 — Copy constructor ==\n";
    {
        Rect rect1(1, 2, 3, 4); // calls the parameterized constructor
        Rect rect2 = rect1;  // call the copy constructor, rect2 is a brand new object with same values as rect1
        Rect rect3(rect1); // same just different syntax
    } 
     ...
     */
          // A destructor is a special method that runs automatically when an object dies
          // (goes out of scope). it is not  called it manually, C++ calls it 
          // In  Rect it just prints a debug message. In general it's used to free
          // resources like heap memory.
    /**
     ...
     */
std::cout << "\n=== TASK 1.6 — When are constructors and destructors called ===\n";
    {
        Rect r1;
        Rect *pR = new Rect(1,2,1,2);   
        {
            Rect r2(r1);
            Rect arRect[2]; //array of two rect objects,called two times
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i);//static local variable is called constructed only once,at i=0 and destructor called once at program termination
                Rect r4(*pR); //copy constructor called 3 times, each r4 destroyed at the end of each loop iteration.
                             
                Rect r5(i,i,i,i);//Parameterized constructor called every iteration
                                 // loops three times so called thre times and each r5 destroyed at end of iteration
    ...
    {
      //All fields (left, top, width, height) are marked private,
      // This means only methods inside the class can access them directly
       // Outside code (like main) cannot ,it must use getters/setters instead.

       // r1.left would give a compile error 
       // becuaseleft is private 
       
      // the controlled interface (get_left(), set_all() etc.)
std::cout << "\n=== TASK 1.7 — Public and private fields ===\n";

        Rect r1;
       // std::cout << r1.left << std::endl; // Error: left is private
       std::cout
        << "Direct access to fields is prohibited due to private visibility."
        << std::endl;
    } 
     ...
     */
std::cout << "\n=== TASK 1.8 — Encapsulation. Getters and setters ===\n";

    {
       Rect r; // default contrusctor 
       std::cout << r.get_left() << "; " << r.get_right() << '\n'; // outputs 0,0
       r.set_all(1, 2, 3, 4); // paramerized contsructor outputs 3 4
       std::cout << r.get_top() << "; " << r.get_bottom() << '\n'; 
    } 
        
     // A regular function stands alone , it has no connection to any object
    //void print_rect(const Rect &r)  must be handed a Rect explicitly

       // A member-function belongs to a class and always has an implicit "this" pointer
       // pointing to the object it was called on
       //r.get_left();  // "this" silently points to `r` inside the method

         // The key differences:
         //  1. Member-functions can access private fields directly — regular functions cannot
         // 2. Member-functions are called on an object (r.get_left()) 
         //    regular functions just take arguments (get_left(r))
         //  3. Member-functions have access to "this" — regular functions do not
     ...
     */
       ...
       // inflate(5)         all 4 sides expand by 5
      // inflate(5, 3)      left+right expand by 5, top+bottom by 3
      // inflate(1,2,3,4)  each side expands independently
    
     // All 3 versions delegate to the 4-argument one — logic lives in one place only.

std::cout << "\n== TASK 1.9 — Overloading and default parameters ==\n";


{
    Rect r(0, 10, 0, 10);

    r.inflate(2);          // calls inflate(int amount)       → expand all sides by 2
    r.inflate(2, 3);       // calls inflate(int dw, int dh)   → x by 2, y by 3
    r.inflate(1, 2, 3, 4); // calls inflate(int,int,int,int)  → each side separately

// move() uses a default parameter value:
//   void move(int dx, int dy = 0)
//   dy = 0 means: if you don't pass it, it defaults to 0
//
// rect.move(1, 2)  moves 1 on X, 2 on Y
// rect.move(1)    moves 1 on X, 0 on Y (dy uses its default)

    r.move(1, 2); // moves right by 1, down by 2
    r.move(1);    // moves right by 1 only — dy defaults to 0
}
    /**
   ...
   std::cout << "\n== TASK 1.10 — Passing objects to and from functions ==\n";
   {
    ...
    // bounding_rect takes r1 and r2 by const reference, so no copies are made
    // Inside bounding_rect a local Rect result is created using the default constructor
    // When returning, the compiler usually uses NRVO, so no copy constructor is called
    Rect r3 = bounding_rect(r1, r2);
        // print_rect takes r3 by reference, so no constructor is called
    print_rect(r3);
    }    // At the end of this block destructors are called for r3, r2, and r1
   }
     ...
     */
      cout << "== field and properties of objects == " << endl;
      {
        Rect test2(5, 4, 6, 3);
        cout << endl;
        cout << "the width of the reactangle is: " << test2.get_width() << endl;
        cout << "the height of the reactangle is: " << test2.get_height() << endl;
        cout << "the Area of the reactangle is: " << test2.get_square() << endl;

        test2.set_width(3);
        test2.set_height(4);
        cout << "the new width of the reactangle is: " << test2.get_width() << endl;
        cout << "the new height of the reactangle is: " << test2.get_height() << endl;
        cout << "the new area of the reactangle is: " << test2.get_square() << endl;
        cout << endl;
    }
    cout << endl;

    cout << "==Object Use Cases. ==" << endl;
     ...
     */
   std::cout << "Starting Barrel" << std::endl;
{
    Barrel alch(100.0, 96.0); // 100L barrel, 96L alcohol (96% concentration)
    Barrel water(100.0, 0.0); // 100L barrel, 0L alcohol (water)

    int iterations = 0;
    while (alch.get_alcohol_concentration() > 50.0 && iterations < 10000) {  //"keep pouring back and forth as long as alcohol is above 50%, but stop no matter what after 10000 iterations
      alch.transfer_from(water, 1.0);  // scoop 1L FROM water INTO alch
      water.transfer_from(alch, 1.0);  // scoop 1L FROM alch INTO water
      iterations++;
    }
    std::cout << "Alcohol concentration dropped below 50% after " << iterations
              << " iterations." << std::endl;
}
    ...
    {
        // Fibonacci calculation using Matrix: F(n+1) F(n) = [1 1; 1 0]^n * [F(1)
    // F(0)] We'll calculate F(10)
    std::cout << "\n= TASK 2.2 — Fibonacci numbers using Matrix exponentiation ==\n";
    Matrix T(2, 2);
    T.set(0, 0, 1);
    T.set(0, 1, 1);
    T.set(1, 0, 1);
    ...
    Matrix res(2); // Initial Identity

    int n = 10;
    for (int i = 0; i < n; ++i) {
      res = res.multiply(T);
    }
    std::cout << "F(10) = " << res.get(0, 1) << " (should be 55)" << std::endl;
    }
     ...
     */
       ...
       cout << " TASK 2.3 Experimenting objects with strings " << endl;
    {
        MyString s1;
        MyString s2("hello there how are you!");
        s2.print();
        cout << (s2.get(1)) << endl;
        s2.set(5, 'z');
        s2.print();
     ...
     */
     ...
     cout << "checking with different situations" << endl;
    {
        MyString s1;
        MyString s2 = s1;
        ...
        /**
       * destructors run from bottom to top, so s4 is destroyed before s3,
       * s3 before s2, and so on when the block ends.
 
        * the problem: s4 = s3 does not copy the actual string content,
        * it only copies the pointer (memory address) stored in s3.data.
         * now both s4 and s3 point to the exact same memory location,
         * neither knows the other exists.
 
         * when s4 is destroyed first, its destructor calls delete[] data,
         * which frees that shared memory. at this point s3.data is now
         * a dangling pointer - it points to memory that has been freed.
 
         * when the destructor reaches s3, it tries to delete[] the same
          * memory again. that memory is already gone, so this is a double
          * free - undefined behaviour that will likely cause a crash.
 
           * the solution is a deep copy constructor - instead of copying
           * just the pointer, allocate brand new memory for the new object
          * and copy the actual characters into it. that way each object
          * owns its own independent memory and freeing one does not
        * affect the other.
 */
    /**
     ...
     */
    std::cout << " TASK 2.4 Text Wrapper " << std::endl;
      {
       int line_width = 40;
       MyString input;
       input.read_line();
     ...
     */
      /*moved the `all` target to the top of the file following standard GNU
 *   practice, so typing just `make` builds the whole project without needing
 *   to know the name of the output binary.
 *
 * - introduced variables: CXX for the compiler, CXXFLAGS for compiler flags,
 *   and SRC and BUILD for source and output paths, so nothing is hardcoded
 *   and changes only need to be made in one place.
 *
 * - added `make clean` to delete all object files and clear the build folder,
 *   making it easy to start a fresh build.
 *
 * - added `make debug` which builds and links the project then launches
 *   the GDB debugger for step by step debugging.
 */
    /**
     ...
     */
      /*Added a build/leaks.out target that builds a version of the program
       with memory leak detection enabled using AddressSanitizer flags
      (-fsanitize=address -g), so any memory leaks or invalid memory access
       are reported at runtime.
 
       Added a leaks target that runs build/leaks.out with a file of
        pre-written user input piped into it, so the program can be tested
        automatically without typing input manually every time.*/
    /**
     ...
     * комментариях старую реализацию.q 
     *
